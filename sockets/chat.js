...
exports.onConnection = function (socket) {
  ...
  socket.on('check credential', function (client) {
    ...
    if (client.mode == 'create') {
      // modeが'create'の場合、すでに対応するroomIdのチャットルームがないかチェックする
    ...
    var message = {
      from: 'システムメッセージ',
      body: client.userName + 'さん入室しました。',
      roomId: client.roomId
    };
    message.date = _formDate(new Date());
  ...
  // ソケットが切断されたと場合、ソケット一覧からソケットを削除する
  socket.on('disconnect', function () {
    socket.get('client', function (err, client) {
      ...
      if (members.length === 0) {
        ...
        var message = {
          from: 'システムメッセージ',
          body: client.userName + 'さんが退出しました',
          roomId: client.roomId
        }
        var shasum = crypto.createHash('sha1');
